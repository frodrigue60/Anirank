...
// BulkCheckAnilistIDs handles POST /api/animes/bulk-check
func (h *CatalogHandler) BulkCheckAnilistIDs(c *fiber.Ctx) error {
	var req struct {
	...
	res, err := h.usecase.BulkCheckAnilistIDs(c.Context(), req.AnilistIDs)
	if err != nil {
		return err
	}

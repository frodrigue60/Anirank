<script lang="ts">
  ...
  async function handleBatchGenerate(e: Event) {
    ...
    try {
      ...
      console.error(err);
      toastState.addToast(`Failed to batch fetch animes: ${err.message || err}`, "error");
    } finally {
      isGenerating = false;
    }
  }
  ...
  >
    <div class="w-full mb-2">
      <h2 class="text-lg font-semibold text-white flex items-center gap-2">
        <span class="material-symbols-outlined text-anirank-primary">auto_fix_high</span>
        Batch Import by Season
      </h2>
      <p class="text-sm text-gray-400">Select parameters to fetch multiple animes at once.</p>
    </div>
...
<div class="bg-anirank-card border border-white/5 rounded-2xl p-6 shadow-xl">
  <div class="mb-4">
    ...
    <p class="text-sm text-gray-400">Search for a specific anime by title to import it immediately.</p>
  </div>

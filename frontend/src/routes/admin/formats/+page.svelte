<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      if (modalMode === "create") {
        const res = await api.post("/admin/taxonomies/formats", payload);
        if (res.data?.data) {
          items = [res.data.data, ...items];
        }
      } else {
        const res = await api.put(
          `/admin/taxonomies/formats/${editingItem.id}`,
          payload,
        );
        if (res.data) {
      ...
      console.error(err);
      errorMsg = err.response?.data?.message || "Failed to save format";
    } finally {
      isSubmitting = false;
    }
  }
  ...
  async function handleDelete(id: number) {
    ...
    try {
      await api.delete(`/admin/taxonomies/formats/${id}`);
...
<div class="space-y-6 animate-fade-in">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-on-surface">Release Formats</h1>
      <p class="text-on-surface-variant/70 mt-1">Manage content distribution formats (TV, Movie, etc.)</p>
    </div>
    <button
      onclick={openCreate}

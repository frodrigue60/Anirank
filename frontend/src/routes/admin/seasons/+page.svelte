<script lang="ts">
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      if (modalMode === "create") {
        const res = await api.post("/admin/taxonomies/seasons", payload);
        if (res.data?.data) {
          if (formCurrent) {
        ...
        const res = await api.put(
          `/admin/taxonomies/seasons/${editingItem.id}`,
          payload,
        );
        if (res.data) {
          if (formCurrent) {
...
<div class="space-y-6 animate-fade-in">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-on-surface">Release Seasons</h1>
      <p class="text-on-surface-variant/70 mt-1">Manage content timeline (Winter, Spring, etc.)</p>
    </div>
    <button
      onclick={openCreate}
    ...
    <div class="bg-surface-container border border-outline-variant rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
      <div class="px-6 py-4 border-b border-outline-variant flex justify-between items-center">
        <h2 class="text-lg font-bold text-on-surface">
          {modalMode === 'create' ? 'Add' : 'Edit'} Season
        </h2>
      ...
      <form onsubmit={handleSubmit} class="p-6 space-y-4">
        ...
        <div class="space-y-1.5">
          <label for="name" class="text-xs font-medium text-on-surface-variant/70 uppercase tracking-widest ml-1">Season Name</label>
          <input
            id="name"
            type="text"
            bind:value={formName}
            required
            placeholder="e.g. Winter, Spring, Summer, Fall"

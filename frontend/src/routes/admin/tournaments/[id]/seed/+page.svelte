<script lang="ts">
  ...
  async function handleSearch() {
    ...
    try {
      const res = await api.get("/admin/songs", {
        params: { search: searchQuery, limit: 10, type: selectedType },
      });
  ...
  async function finalizeSeeding() {
    ...
    if (method === "manual" && selectedSongs.length < tournament.size) {
      alert(
        `Please select ${tournament.size} songs (current: ${selectedSongs.length})`,
      );
      return;
    }
...
<div class="seeding-wizard p-6 max-w-6xl mx-auto">
  ...
  {:else if tournament}
    ...
    <div class="flex items-center gap-4 mb-10 overflow-x-auto">
      {#each [1, 2, 3] as s}
        <div class="flex items-center gap-2">
          ...
          <span
            class="whitespace-nowrap {step === s ? 'font-bold' : 'opacity-50'}"
          >
    ...
    {#if step === 1}
      <div
        class="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in duration-300"
      >
        <div class="space-y-6">
            ...
            <div class="flex items-start gap-4">
              ...
              <div>
                ...
                <div class="text-sm opacity-60">
                  Picks random songs based on base tournament filters.
                </div>
              </div>
            </div>
          </label>
          ...
          <label class="method-card {method === 'filtered' ? 'active' : ''}">
            ...
            <div class="flex items-start gap-4">
              ...
              <div>
                ...
                <div class="text-sm opacity-60">
                  Apply specific year, season or genre filters first.
                </div>
              </div>
            </div>
          </label>
          ...
          <label class="method-card {method === 'manual' ? 'active' : ''}">
            ...
            <div class="flex items-start gap-4">
              ...
              <div>
                ...
                <div class="text-sm opacity-60">
                  Search and pick songs one by one. Supports saving progress.
                </div>
              </div>
            </div>
          </label>
        ...
        >
          ...
          <div class="space-y-4">
            <div>
              <label for="year-select" class="block text-sm opacity-50 mb-1"
                >Year</label
              >
              <select
                id="year-select"
            ...
            <div>
              <label for="season-select" class="block text-sm opacity-50 mb-1"
                >Season</label
              >
              <select
                id="season-select"
            ...
            <div>
              <label for="genre-select" class="block text-sm opacity-50 mb-1"
                >Genre</label
              >
              <select
                id="genre-select"
            ...
            <div>
              <label for="type-select" class="block text-sm opacity-50 mb-1"
                >Theme Type</label
              >
              <select
                id="type-select"
            ...
            <div>
              <label for="priority-select" class="block text-sm opacity-50 mb-1"
                >Priority</label
              >
        ...
        <div class="manual-panel grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            >
              ...
              {:else if searchResults.length > 0}
                {#each searchResults as song}
                  ...
                  >
                    ...
                    <div class="flex-1">
                      <div
                        class="text-[10px] text-primary font-bold uppercase tracking-wider mb-0.5"
                      >
                        {song.anime?.title}
                      </div>
              ...
              {:else}
                <div class="p-8 text-center opacity-50">
                  Start typing to find anime songs...
                </div>
              {/if}
            </div>
          </div>
          ...
          >
            <div class="flex justify-between items-center mb-6">
              ...
              <span
                class="bg-primary/20 text-primary px-3 py-1 rounded-full font-bold text-sm"
              >
      ...
      {:else}
        ...
        >
          ...
          <p class="opacity-60 mb-8 max-w-md mx-auto">
            The system will automatically find {tournament.size} songs matching your
            criteria. Method:
            <span class="text-primary font-bold"
              >{method === "random"
                ? "Pure Random"
                : "Filtered Discovery"}</span
            >
          </p>
          <div class="flex justify-center gap-4">
            ...
            <button
              type="button"
              class="px-8 py-3 rounded-xl bg-primary font-bold"
              on:click={nextStep}>Final Confirmation →</button
            >
          </div>
        </div>
      {/if}
        ...
        >
          ...
          <div class="aspect-video w-full bg-black relative">
            ...
            {:else}
              <div
                class="w-full h-full flex items-center justify-center opacity-40"
              >
                No variants or videos available
              </div>
            {/if}
          </div>
          ...
          <div class="p-8">
            <div class="flex gap-6 mb-6">
              <img
                src={previewSong.anime?.cover_url}
                alt=""
                class="w-24 h-32 object-cover rounded-xl shadow-2xl"
              />
              <div class="flex-1 pt-2">
                <h3 class="text-2xl font-bold mb-1">
                  {previewSong.song_romaji}
                </h3>

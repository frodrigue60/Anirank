...
<div class="mb-8 flex items-center justify-between">
  <div class="flex items-center gap-4">
    ...
    <div>
      <h1 class="text-3xl font-bold tracking-tight text-white mb-0.5">User Details</h1>
      <p class="text-gray-400 text-sm">Managing account and permissions for <span class="text-anirank-primary font-medium">{user.name}</span></p>
    </div>
  </div>
  ...
  <div class="flex items-center gap-2">
    <a
      ...
      class="px-4 py-2 bg-anirank-primary/10 hover:bg-anirank-primary/20 text-anirank-primary border border-anirank-primary/20 rounded-xl transition-all font-medium flex items-center gap-2"
    >
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
  ...
  <div class="lg:col-span-2 space-y-8">
    ...
    <!-- Hero Section -->
    <div class="relative bg-anirank-card border border-white/5 rounded-3xl overflow-hidden group">
      <!-- Banner -->
      ...
      <div class="px-8 pb-8 -mt-12 relative flex flex-col sm:flex-row items-end gap-6">
        <div class="relative shrink-0">
          <div class="w-32 h-32 rounded-3xl overflow-hidden border-4 border-anirank-bg shadow-2xl bg-white/5">
            {#if user.avatar_url}
              <img src={user.avatar_url} alt={user.name} class="w-full h-full object-cover" />
            {:else}
              <div class="w-full h-full flex items-center justify-center text-4xl font-bold text-gray-500 uppercase">
                {user.name.charAt(0)}
              </div>
            {/if}
          </div>
          <div class="absolute -bottom-2 -right-2 bg-anirank-primary text-white w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-sm shadow-xl border-4 border-anirank-bg">
            {user.level}
          </div>
        </div>
        ...
        <div class="flex-1 pb-2">
          <div class="flex items-center gap-3 mb-1">
            <h2 class="text-3xl font-bold text-white leading-none">{user.name}</h2>
            {#if user.roles?.some((r: any) => r.slug === 'admin' || r.slug === 'owner')}
               <span class="material-symbols-outlined text-anirank-primary text-xl" title="Verified Staff">verified</span>
            {/if}
          </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="bg-anirank-card border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors">
        <div class="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-2">Total XP</div>
        <div class="text-2xl font-bold text-white flex items-baseline gap-1">
          {user.xp.toLocaleString()}
          <span class="text-xs text-anirank-primary">pts</span>
        </div>
      </div>
      <div class="bg-anirank-card border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors">
        <div class="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-2">Followers</div>
        <div class="text-2xl font-bold text-white">{user.followers_count || 0}</div>
      </div>
       <div class="bg-anirank-card border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors">
        <div class="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-2">Following</div>
        <div class="text-2xl font-bold text-white">{user.following_count || 0}</div>
      </div>
      <div class="bg-anirank-card border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors">
        <div class="text-gray-500 text-xs font-semibold uppercase tracking-wider mb-2">Ratings</div>
        <div class="text-2xl font-bold text-white">{user.ratings_count || 0}</div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
       ...
       <div class="bg-anirank-card border border-white/5 rounded-2xl overflow-hidden">
         <div class="px-6 py-4 border-b border-white/5 bg-white/2">
           <h3 class="text-sm font-bold text-white flex items-center gap-2">
             <span class="material-symbols-outlined text-anirank-primary text-[18px]">account_circle</span>
             Account Information
           </h3>
         </div>
         <div class="p-6 space-y-4">
            <div class="flex justify-between items-center bg-white/2 px-4 py-3 rounded-xl border border-white/5">
              <span class="text-gray-400 text-sm">Joined Date</span>
              <span class="text-white text-sm font-medium">{new Date(user.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </div>
            <div class="flex justify-between items-center bg-white/2 px-4 py-3 rounded-xl border border-white/5">
             ...
             <div class="flex justify-between items-center bg-white/2 px-4 py-3 rounded-xl border border-white/5">
              <span class="text-gray-400 text-sm">Score Format</span>
              <span class="text-white text-sm font-medium uppercase">{user.score_format || 'Standard'}</span>
            </div>
         </div>
       </div>
       ...
       <div class="bg-anirank-card border border-white/5 rounded-2xl overflow-hidden">
         <div class="px-6 py-4 border-b border-white/5 bg-white/2">
           <h3 class="text-sm font-bold text-white flex items-center gap-2">
             <span class="material-symbols-outlined text-anirank-primary text-[18px]">hub</span>
             Linked Accounts
           </h3>
         </div>
         <div class="p-6 space-y-4">
            <!-- AniList -->
             <div class="flex justify-between items-center bg-white/2 px-4 py-3 rounded-xl border border-white/5">
              <div class="flex items-center gap-3">
                ...
                <span class="text-white text-sm font-medium">AniList</span>
              </div>
              {#if user.anilist_id}
              ...
              {:else}
                <span class="text-gray-500 text-[10px] font-bold uppercase tracking-widest bg-white/5 px-2 py-0.5 rounded">Not Linked</span>
              {/if}
            </div>
  ...
  <div class="space-y-8">
     <!-- Roles Card -->
     <div class="bg-anirank-card border border-white/5 rounded-3xl p-6">
       ...
       <div class="flex flex-wrap gap-2">
         {#each user.roles || [] as role}
           <div class="px-3 py-1.5 bg-anirank-primary/10 border border-anirank-primary/20 rounded-xl text-anirank-primary text-xs font-bold uppercase tracking-wider">
             {role.name}
           </div>
         {:else}
           <div class="w-full py-4 text-center text-gray-500 text-sm italic bg-white/2 rounded-2xl border border-white/5">
             No roles assigned
           </div>
         {/each}
       </div>
     ...
     <div class="bg-anirank-card border border-white/5 rounded-3xl p-6">
       <div class="flex items-center justify-between mb-6">
         ...
         <span class="text-xs text-gray-500 font-mono">{user.badges?.length || 0} Total</span>
       </div>
       <div class="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-4 gap-3">
         {#each user.badges || [] as badge}
           <div class="aspect-square bg-white/5 rounded-2xl flex items-center justify-center p-2 border border-white/5 hover:border-white/20 transition-all hover:-translate-y-1 relative group" title={badge.name}>
             {#if badge.icon_url}
             ...
             {/if}
             <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-black rounded text-[10px] text-white opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10 shadow-2xl">
               {badge.name}
             </div>
           </div>
         {:else}
           <div class="col-span-full py-4 text-center text-gray-500 text-sm italic bg-white/2 rounded-2xl border border-white/5">
             No badges earned
           </div>
         {/each}
       </div>
       ...
       <div class="space-y-3">
         {#if newPassword}
           <div class="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl mb-4" in:slide>
             ...
             <div class="flex items-center gap-2">
                <code class="flex-1 bg-black/40 px-3 py-2 rounded-xl text-white font-mono text-lg tracking-wider border border-white/5">{newPassword}</code>
         ...
         <button 
           ...
           class="w-full px-4 py-3 bg-white/5 hover:bg-white/10 text-white rounded-2xl transition-all border border-white/10 text-sm font-semibold flex items-center gap-3 disabled:opacity-50"
         >

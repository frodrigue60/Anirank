  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      if (modalMode === "create") {
        ...
        toastState.addToast(`Year ${modalMode === 'create' ? 'created' : 'updated'} successfully`, 'success');
      }
      showModal = false;
    } catch (err: any) {
      console.error(err);
      const msg = err.response?.data?.message || `Failed to ${modalMode} year`;
      errorMsg = msg;
  ...
  async function toggleCurrent(id: number) {
    try {
      ...
      if (res.status === 200 || res.status === 204) {
        ...
        toastState.addToast("Primary year updated", "success");
      }
    } catch (err: any) {
      console.error(err);
            ...
            <tr class="hover:bg-white/2 transition-colors group">
              ...
              <td class="px-6 py-4">
                <button
                  ...
                  title={item.current ? "Primary Year" : "Set as Primary"}
                >

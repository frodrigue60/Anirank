...
export const load = async ({ params, parent }: { params: { slug: string }, parent: () => Promise<any> }) => {
    ...
    if (!user) {
        return { songs: { data: [], current_page: 1, last_page: 1 } };
    }

    try {
        const res = await api.post(`/users/favorites/themes`, { user_id: user.id, page: 1 });
        return {
            songs: res.data.songs || { data: [], current_page: 1, last_page: 1 }
        };
    } catch (e: any) {
        ...
        return {
            songs: { data: [], current_page: 1, last_page: 1 }
        };
    }
};

...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-white tracking-tight">Edit Year Entry</h1>
            <p class="text-zinc-400 mt-1">Updating entry for year <span
                    class="text-blue-400 font-semibold">{{ $year->name }}</span></p>
        </div>

        {{-- Form Card --}}
        <div class="bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-3xl shadow-xl overflow-hidden p-8">
            <form method="post" action="{{ route('admin.years.update', $year->id) }}" class="space-y-6">
                @csrf
                @method('PUT')

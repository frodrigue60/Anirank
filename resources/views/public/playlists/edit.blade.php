...
@section('meta')
    <title>Edit Playlist | {{ config('app.name') }}</title>
@endsection
    ...
    <div class="max-w-[1440px] mx-auto px-4 md:px-8 py-10 md:py-8 flex flex-col gap-12">
        <div class="max-w-2xl mx-auto w-full">
            ...
            <div class="flex flex-col gap-4 mb-8 text-center">
                ...
                <h1 class="text-3xl font-black tracking-tight text-white">Edit Playlist</h1>
                <p class="text-white/40 text-sm">Update your playlist details.</p>
            </div>

            {{-- Form Card --}}
            <div class="bg-surface-dark/30 p-8 rounded-2xl border border-white/5 shadow-2xl backdrop-blur-md">
                <form action="{{ route('playlists.update', $playlist->id) }}" method="post" class="flex flex-col gap-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="relative group">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="name" name="name" required value="{{ $playlist->name }}"
                    ...
                    <div class="relative group">
                        ...
                        <div class="relative">
                            <textarea id="description" name="description" rows="4" maxlength="255"
                                class="w-full bg-surface-darker border border-white/10 rounded-lg p-4 text-sm text-white focus:outline-none focus:border-primary/50 transition-all placeholder:text-white/20 resize-none">{{ $playlist->description }}</textarea>
                        </div>
